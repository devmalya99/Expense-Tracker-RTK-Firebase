...
<header className="bg-white border-b sticky top-0 z-10 p-4">
    ...
    <div className="border-2 bg-blue-500 border-gray-200 p-4 rounded-md  shadow-sm">
        <ul>
          {Array.isArray(expenses) && expenses.length > 0 && ( // Check for array and non-empty
            expenses.map((expense) => (
              <div 
              className='flex border-2 mt-4 border-gray-200 p-5 rounded-md bg-white shadow-sm'
              key={expense.id}>
                <li>
                  <p>{expense.title}</p>
                  <p>{expense.amount}</p>
          ...
          {Array.isArray(expenses) && expenses.length === 0 && ( // Show message if empty
